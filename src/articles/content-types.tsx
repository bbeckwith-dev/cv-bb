...
function ScreenshotFigure({ src, alt, summary, width, height, className }: { src: string; alt: string; summary: string; width?: number; height?: number; className?: string }) {
  ...
  return (
    ...
    >
      ...
      >
        <p className="text-xs text-foreground leading-relaxed text-center">{summary}</p>
      </div>
    </figure>
  )
}
...
export function ScreenshotCaption({ caption, editorId }: ScreenshotCaptionProps) {
  return (
    <EditorLabel name="ScreenshotCaption" id={editorId}>
      <p className="text-xs text-muted-foreground mb-6 -mt-4 px-1">{caption}</p>
    </EditorLabel>
  )
}
