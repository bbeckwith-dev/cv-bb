...
// Map article id → i18n content for JSON-LD generation (populated when articles are added)
...
let enPage = indexHtml
  ...
  .replace(/<link rel="canonical" href="[^"]*" \/>/, '<link rel="canonical" href="https://cv.brentbeckwith.com/en" />')
  .replace(/<meta property="og:url" content="[^"]*" \/>/, '<meta property="og:url" content="https://cv.brentbeckwith.com/en" />')
  ...
  .replace(/<meta name="twitter:url" content="[^"]*" \/>/, '<meta name="twitter:url" content="https://cv.brentbeckwith.com/en" />')
...
const aboutPersonProfile = {
  ...
  mainEntity: {
    ...
    knowsAbout: [
      ...
      { '@type': 'Thing', name: 'System Administration', url: 'https://en.wikipedia.org/wiki/System_administrator' },
      { '@type': 'Thing', name: 'Python', url: 'https://en.wikipedia.org/wiki/Python_(programming_language)' },
      { '@type': 'Thing', name: 'Artificial Intelligence', url: 'https://en.wikipedia.org/wiki/Artificial_intelligence' },
      { '@type': 'Thing', name: 'Web Hosting', url: 'https://en.wikipedia.org/wiki/Web_hosting_service' },
      { '@type': 'SoftwareApplication', name: 'Claude API', url: 'https://docs.anthropic.com' },
    ],
    hasCredential: [],
    alumniOf: [
      { '@type': 'EducationalOrganization', name: 'ITT Technical Institute' },
      { '@type': 'EducationalOrganization', name: 'The Odin Project', url: 'https://www.theodinproject.com' },
    ],
    sameAs: [
      'https://www.linkedin.com/in/brent-beckwith',
      'https://github.com/bbeckwith-dev',
    ],
    address: { '@type': 'PostalAddress', addressLocality: 'Madison', addressRegion: 'OH', addressCountry: 'US' },
  },
};
...
function buildAboutJsonLd(lang: 'es' | 'en', pageUrl: string, faq: readonly { q: string; a: string }[]) {
  ...
  const profile = {
    ...
    inLanguage: lang,
    mainEntity: { '@id': 'https://cv.brentbeckwith.com/#person' },
  };
  return {
...
for (const lang of ['es', 'en'] as const) {
  ...
  const altUrl = `https://cv.brentbeckwith.com/${altSlug}`;
  ...
  const hreflangLinks = `<link rel="alternate" hreflang="${lang}" href="${url}" /><link rel="alternate" hreflang="${altLang}" href="${altUrl}" /><link rel="alternate" hreflang="x-default" href="https://cv.brentbeckwith.com/sobre-mi" />`;
...
function buildArticlePage(
  ...
  const xDefaultHref = `https://cv.brentbeckwith.com/${config.xDefaultSlug || config.slugs.es}`;
  ...
  let result = indexHtml
    ...
    .replace(/<meta property="og:image" content="[^"]*" \/>/, `<meta property="og:image" content="${esc(config.ogImage || 'https://cv.brentbeckwith.com/og-image.webp')}" />`)
  ...
  if (seoMeta) {
    const articleMetaTags = [
      ...
      `<meta property="article:author" content="https://www.linkedin.com/in/brent-beckwith" />`,
  ...
  if (seoMeta && i18n) {
    ...
    if (t) {
      const jsonLd = buildArticleJsonLd({
        lang,
        url: `https://cv.brentbeckwith.com/${slug}`,
        altUrl: `https://cv.brentbeckwith.com/${altSlug}`,
...
for (const [lang, slug, altSlug] of [['es', 'privacidad', 'privacy'], ['en', 'privacy', 'privacidad']] as const) {
  ...
  const hreflangLinks = `<link rel="alternate" hreflang="${lang}" href="${url}" /><link rel="alternate" hreflang="${altLang}" href="${altUrl}" /><link rel="alternate" hreflang="x-default" href="https://cv.brentbeckwith.com/privacidad" />`;
